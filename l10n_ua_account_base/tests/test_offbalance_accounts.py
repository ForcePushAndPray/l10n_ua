...
class TestOffBalanceAccounts(TransactionCase):
    ...
    def test_offbalance_entry_must_still_balance(self):
        ...
        застосовується до будь-якої проводки, тож провести Дт 01 без кредиту
        неможливо. Тест фіксує обмеження, щоб воно не з'їхало непоміченим.

        Другий рахунок тут — технічна пара, а не бухгалтерська істина: у Наказі
        № 291 контррахунку для класу 0 немає, 09 має власний зміст. Щоб
        орендоване майно обліковувалось як у 1С, потрібні односторонні записи —
        це вже зміна рушія, заведено окремо (#256).
        """
