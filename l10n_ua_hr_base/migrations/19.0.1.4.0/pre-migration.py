"""Report, before anything commits, the rows that will refuse the new guarantee.

Read-only. It is about a guarantee that may fail to be created.
...
The other consequence of this release — `date_to` turning from "Effective
Until" into "Position Discontinued" — is reported from the post-migration of
19.0.1.6.1 instead. That scan has to reach the chatter of the lines it is
about, which needs an ORM this script does not have, and it has to run on the
databases that already went through 19.0.1.4.0, which a script filed here never
would.
"""
